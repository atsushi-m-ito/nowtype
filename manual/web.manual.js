"use strict";
/* 
ES2015 テンプレート文字列で変数に複数行文字列を格納
バッククォートで囲えばよい。
バッスラッシュとバッククォートはエスケープシーケンスで記述
*/

const nt_web_manual_text = `
...
一般的にMarkdownでは文章の纏まりは、段落となり、改行すると次の段落になります。

ただし、段落の最初の文字列がシャープ\`#\`と半角スペースで始まる時、続く文字列は段落ではなく見出しとみなされます。\`#\`の数によって、見出しのレベルを6段階に設定できます。次の例を見てください。
\`\`\`
段落の文章〇〇〇〇
...
1. リストC
\`\`\`
ここで、数値に意味はなく、連番である必要アありません。逆に何かしら意味のある数値を書いても、表示には反映されません。表示の際には自動で連番になり、次のように表示されます。
...
- 各括弧\`[\`を入力すると、サンプルのMarkdownテキストが入力されます。強調表示や数式ブロックの様に入力モードになっていますので、目的のURLリンクに書き換えてください。
...
- エクスクラメーションマーク\`!\`に続いて各括弧\`[\`を入力すると、サンプルのMarkdownテキストが入力されます。強調表示や数式ブロックの様に入力モードになっていますので、目的の画像のパスに書き換えてください。相対パスで大丈夫です。
...
- 各括弧\`[\`を入力すると上述の様にリンクが自動生成されますが、そのまま\`^\`を入力すると、引用リンクになります。
